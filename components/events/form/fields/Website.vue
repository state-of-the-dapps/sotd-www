<template>
  <div class="item" :class="errors && errors.length > 0 ? '--has-errors' : ''">
    ...
    <p class="help">A URL to this event's website</p>
  </div>
</template>
...
<script>
  ...
  export default {
    computed: {
      errors () {
        return this.$store.getters['events/form/websiteUrlErrors']
      },
      url: {
        get () {
          return this.$store.getters['events/form/websiteUrl']
        },
        set (value) {
          const field = {
            name: 'website',
          ...
          this.$store.dispatch('events/form/setSiteUrl', field)
        }
      }
    },
    methods: {
